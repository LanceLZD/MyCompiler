...
void updateInstrInfo(uint32_t keys, uint32_t values) {
    if (instr_map.count(keys) == 0)
    	instr_map.insert(std::pair<uint32_t, uint32_t>(keys, values));
    else
    	instr_map[keys] = instr_map[keys] + values;
