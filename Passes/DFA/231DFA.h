...
class DataFlowAnalysis {
		...
		void initializeBackwardMap(Function * func) {
            ...
            for (Function::iterator bi = func->begin(), e = func->end(); bi != e; ++bi) {
				...
				if (isa<PHINode>(firstInstr)) {
					...
					if (block->getFirstNonPHI()->getOpcode()==1)
                        ret = block->getFirstNonPHI();
				}
			...
			addEdge(nullptr, ret, &InitialState);
  ...
  public:
    ...
    unsigned getIndexByInstr(Instruction * instr) {
        return InstrToIndex[instr];
    }

    Instruction* getInstrByIndex(unsigned index) {
    ...
    void runWorklistAlgorithm(Function * func) {
        ...
        for (auto val = IndexToInstr.begin(); val != IndexToInstr.end(); val++) {
            if (val->first != 0)
                worklist.push_front(val->first);
        }

    	// (3) Compute until the work list is empty

    	while (!worklist.empty()) {
            unsigned tmp = worklist.front();
            worklist.pop_front();
            std::vector<unsigned> *IncomingEdges = new std::vector<unsigned>();
            getIncomingEdges(tmp, IncomingEdges);
            std::vector<unsigned> *OutgoingEdges = new std::vector<unsigned>();
            getOutgoingEdges(tmp, OutgoingEdges);
            std::vector<Info*> *Infos = new std::vector<Info*>();
            for (unsigned i=0;i<OutgoingEdges->size();i++) {
                Info* iniOutInfo = new Info();
                Infos->push_back(iniOutInfo);
            }

            flowfunction(IndexToInstr[tmp], *IncomingEdges, *OutgoingEdges, *Infos);

            for (unsigned i=0;i<OutgoingEdges->size();i++) {
                if (!Info::equals((*Infos)[i], EdgeToInfo[std::make_pair(tmp, (*OutgoingEdges)[i])])) {
                    Info *joined = new Info();
                    Info::join((*Infos)[i],EdgeToInfo[std::make_pair(tmp, (*OutgoingEdges)[i])],joined);
                    EdgeToInfo[std::make_pair(tmp, (*OutgoingEdges)[i])] = joined;
